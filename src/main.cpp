...
int main() {
  ...
  rec_cte.open ("record_cte_delay.csv");
  rec_wp.open ("record_waypoints_delay.csv");
  rec_act.open ("record_actuators_delay.csv");
  //clock_t time_req;
  //time_req = clock();
  auto start = std::chrono::high_resolution_clock::now();

  h.onMessage([&mpc, &rec_count, &rec, &rec_cte, &rec_wp, &rec_act, &start](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          //double cte_test = py - ptsy[0];
          //std::cout<<"cte0 = "<<cte0<<" cte_test = "<<cte_test<<"\n";
          ...
          if (rec_count < 300){
          	rec_cte << duration.count() << "," << cte0 <<",\n";
          	rec_wp << px << "," << py << "," << psi << "," << v << ",\n";
          	rec_act << duration.count() << "," << steer_value << "," << throttle_value << ",\n";
          	rec_count += 1;
